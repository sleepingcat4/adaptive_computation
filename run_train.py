...
def main(cfg):
    ...
    if cfg.LOAD:
        ...
        optimizer = torch.optim.Adam([p for p in model.parameters() if p.requires_grad], lr=cfg.SOLVER.LR)
        load_checkpoint(load_path, model, optimizer, cfg, device)
    else:
        model = BaseACT(cfg).to(device)
        optimizer = torch.optim.Adam([p for p in model.parameters() if p.requires_grad], lr=cfg.SOLVER.LR)
        if cfg.SOLVER.USE_SCHEDULER:
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=10, threshold=0.0, threshold_mode='rel', cooldown=10, min_lr=1e-5)

    cfg.freeze()

    train(cfg=cfg, model=model, optimizer=optimizer, scheduler=scheduler, data_manager=resolve_data_manager(cfg), device=device)
